...
import { CASOS, crearFixture, borrarFixture, type Caso, type Fixture } from './casos'
...
// Cada caso corre en una sesión nueva que se borra al terminar (salvo los que declaran
// prepararSesion, que reutilizan una sesión de la fixture y no se borran hasta el final).
...
async function correr(c: Caso, intento: number, fx: Fixture): Promise<Resultado> {
  const t0 = Date.now()
  const sesionPropia = !c.prepararSesion
  const sid = c.prepararSesion ? await c.prepararSesion(fx) : (await call(R_SES, 'POST', { id: fx.solucionId }, { tipo: 'LIBRE' })).json.id
  let ult: any = null
  try {
    for (const q of c.turnos) { await call(R_MSG, 'POST', { id: fx.solucionId, sid }, { contenido: q }); ult = await esperar(fx.solucionId, sid) }
  ...
  // La sesión propia del caso se limpia; la reutilizada (prepararSesion) la borra borrarFixture al final.
  if (sesionPropia) {
    await prisma.proyectoMensaje.deleteMany({ where: { sesionId: sid } })
    await prisma.proyectoSesion.delete({ where: { id: sid } }).catch(() => {})
  }
...
async function main() {
  ...
  // Por caso (útil con EVAL_N>1 para ver cuáles fallan de forma intermitente)
  const porCaso: Record<string, [number, number]> = {}
  for (const r of res) { const x = (porCaso[r.caso] ??= [0, 0]); x[1]++; if (r.ok) x[0]++ }
  const inestables = Object.entries(porCaso).filter(([, [a, b]]) => a > 0 && a < b)
  if (N > 1 && inestables.length) { console.log('\nIntermitentes:'); inestables.forEach(([k, [a, b]]) => console.log(`  ${k}: ${a}/${b}`)) }
  ...
  fs.writeFileSync(archivo, JSON.stringify({ fecha: new Date().toISOString(), n: N, tasa, porCategoria: porCat, porCaso, resultados: res }, null, 2))
