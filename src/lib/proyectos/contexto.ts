...
export const DOC_HERRAMIENTA: Record<string, string> = { prd: 'prd', diseno: 'diseno_tecnico', plan_ejec: 'plan_ejecucion', cronograma: 'cronograma', plan_trabajo: 'plan_trabajo', memoria: 'memoria' }
...
export async function construirContexto(
  ...
  for (const def of DEFS) {
    ...
    let t = texto
    if (t.length > def.max) { t = corta(t, def.max) + `\n[RECORTADO por tope: hay más contenido; léelo con ${DOC_HERRAMIENTA[def.clave] ? `leer_documento(nombre="${DOC_HERRAMIENTA[def.clave]}")` : def.clave === 'backlog' ? 'consultar_backlog' : 'las herramientas'}]`; f.estado = 'recortada'; f.nota = [f.nota, `tope de ${def.max} caracteres`].filter(Boolean).join(' · ') }
    f.chars = t.length
    fuentes.push(f)
